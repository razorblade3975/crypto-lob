...
public:
    ...
    using Tree = boost::intrusive::set<
        Node, 
    ...
    explicit BookSide(PoolType& pool, std::size_t initial_capacity = 4096)
        : pool_(pool), tree_(), index_() {
    ...
    UpdateResult apply_update(Price px, std::uint64_t qty, std::uint32_t seq) {
        ...
        if (it == index_.end()) {
            ...
            assert(insert_result.second);  // insertion must succeed for new price
            index_[px] = node;
            ...
            if (tree_.size() == top_n_depth_ || 
                (tree_.size() > top_n_depth_ && result.affects_top_n)) {
                update_nth_price();
            }
    ...
    [[nodiscard]] const Node* kth_level(std::size_t k) const noexcept {
        if (k >= tree_.size()) return nullptr;
    ...
    [[nodiscard]] bool affects_top_n(Price px) const noexcept {
        if (tree_.size() < top_n_depth_) return true;
...
private:
    ...
    [[nodiscard]] bool validate_bst_property() const {
        ...
        for (auto it = ++tree_.begin(); it != tree_.end(); ++it) {
            if constexpr (std::is_same_v<Comparator, AskComparator>) {
                if (!(last_price < it->price)) return false;
            } else {
                if (!(last_price > it->price)) return false;
            }
            last_price = it->price;
