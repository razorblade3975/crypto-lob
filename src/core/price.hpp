...
private:
    ...
    static constexpr std::pair<value_type, value_type> normalize_for_operation(
        const Price& a, const Price& b) noexcept {
...
public:
    ...
    constexpr explicit Price(value_type raw_value, int64_t scale) noexcept 
        : value_(raw_value), scale_(scale) {}
    ...
    [[nodiscard]] constexpr value_type raw_value() const noexcept { return value_; }
    [[nodiscard]] constexpr int64_t scale() const noexcept { return scale_; }
    ...
    static constexpr Price zero() noexcept { return Price(0, HIGH_SCALE); }
