...
struct PoolTestParams {
    ...
    friend std::ostream& operator<<(std::ostream& os, const PoolTestParams& params) {
        return os << "capacity_" << params.capacity 
                  << "_cache_" << params.cache_size 
                  << "_batch_" << params.batch_size;
    }
};
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        PoolTestParams{1, 1, 1},           // Minimal configuration
...
TEST_P(ParameterizedMemoryPoolTest, MixedSizeAllocationStress) {
    ...
    MemoryPool<LargeObject> large_pool(std::max(params.capacity / 4, size_t{1}), PoolDepletionPolicy::THROW_EXCEPTION, config_);
                ...
                try {
                    switch (size_choice) {
                        case 0: {
                            ...
                            if (obj) allocated_objects.emplace_back(obj);
                            break;
                        }
                        case 1: {
